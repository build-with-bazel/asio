...
class server {
 public:
  server(asio::io_context& io_context, unsigned short port)
    ...
    const std::string working_dir(std::getenv("BUILD_WORKING_DIRECTORY") ?: "");
    context_.use_certificate_chain_file(
        working_dir.size() ? working_dir + "/server.pem" : "server.pem");
    context_.use_private_key_file(
        working_dir.size() ? working_dir + "/server.pem" : "server.pem",
        asio::ssl::context::pem);
    context_.use_tmp_dh_file(working_dir.size() ? working_dir + "/dh2048.pem"
                                                : "dh2048.pem");
